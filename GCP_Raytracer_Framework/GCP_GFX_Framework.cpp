...
void GCP_Framework::Render(Camera& camera)
{
	// sanity check that Init() has been called
		assert(_mainBuffer != nullptr);
	...
	while (go)
	{
		...
		while (SDL_PollEvent(&incomingEvent))
		{
			...
			case SDL_KEYDOWN:
				...
				{
				case SDLK_DOWN:
					camera.ChangeCameraAngleY(-0.1f) ;
					break;
				case SDLK_UP:
					camera.ChangeCameraAngleY(0.1f);
					break;
				case SDLK_LEFT:
					camera.ChangeCameraAngleX(-0.1f);
					break;
				case SDLK_RIGHT:
					camera.ChangeCameraAngleY(0.1f);
					break;
		...
		if (cmdRotateRight) camera.ChangeCameraAngleX(0.1f);
		if (cmdRotateLeft) camera.ChangeCameraAngleX(-0.1f);
		if (cmdRotateUp) camera.ChangeCameraAngleY(0.1f);
		if (cmdRotateDown) camera.ChangeCameraAngleY(-0.1f);
